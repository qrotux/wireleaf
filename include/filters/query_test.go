...
func TestParseQueryErrors(t *testing.T) {
	...
	for name, in := range map[string]string{
		...
		"nested bracket":   "where[na[me][eq]=x",
		"trailing junk":    "where[name]x=1",
		"empty member":     "where[age][in]=1,,2",
		"unknown op":       "where[name][like]=x",
		"dup key":          "where[name][eq]=x&where[name][eq]=y",
		"bad index":        "where[or][a][name][eq]=x",
		"index leading 0":  "where[or][00][name][eq]=x",
		"index signed":     "where[or][+1][name][eq]=x",
		"group no path":    "where[or][0]=x",
...
func TestParseQueryErrorPaths(t *testing.T) {
	...
	for name, tc := range map[string]struct{ in, want string }{
		...
		"nested bracket":   {"where[na[me][eq]=x", "where[na[me][eq]"},
		"trailing junk":    {"where[name]x=1", "where[name]x"},
		"empty member":     {"where[age][in]=1,,2", "age:"},
		...
		"group no path":    {"where[or][0]=x", "where[or][0]"},
