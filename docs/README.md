...
| --- | --- | --- |
| [graph.md](graph.md) | `graph` | The typed builder: nodes, wires, edge declaration, `Compile` validation (Finding taxonomy), fetch binds, the request-scoped `Loader`, the `Inputs` declaration a list operation is compiled from, and the `loadertest` conformance harness for fetchers. |
| [include.md](include.md) | `include` | The `?include=` engine: parse → resolve → materialize pipeline, the `Hydrate*` facades and the bound `Hydrator` over a `ListFetcher`, `Ctx`, `Ctx.Policies` and the `Ctx.Request` snapshot, list `Inputs` and `ResolveInputs`, the filter model with `ParseFilterJSON` / `ParseFilterQuery` / `FilterOpsFor`, fetcher contracts (`FetchByIDs`/`FetchByParents`, the limit+1 probe), byte-exactness guarantees, error taxonomy. |
