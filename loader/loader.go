...
// errNilFetch is returned by Warm on a Loader built without New, so a
// missing fetch fails loudly instead of negative-caching every key.
var errNilFetch = errors.New("loader: Loader has nil fetch (use New)")

// errFetchPanicked settles entries whose fetch panicked, so waiters unblock
// with a miss instead of hanging forever.
var errFetchPanicked = errors.New("loader: fetch panicked")
...
// is NOT registered on a graph.Builder and holds no data of its own; every
// cached value lives on the *include.Ctx of one request and dies with it.
//
...
// (see New's last bullet). Get never fetches; it only reports what Warm
...
// check included, so a Loader built without New does not fail there.
//
...
// New returns a Loader that batch-fetches values with fetch.
//
// The fetch contract:
...
//     ignored (they would never be read anyway); the graph/loadertest harness
...
func New[K comparable, V any](fetch func(c *include.Ctx, keys []K) (map[K]V, error)) *Loader[K, V] {
...
// state is one Loader's per-request state, stored on the Ctx under the
// *Loader pointer.
...
// perRequest returns this loader's state for c, creating it on first use.
func (l *Loader[K, V]) perRequest(c *include.Ctx) *state[K, V] {
	st := c.State(l, func() any {
		return &state[K, V]{entries: make(map[K]*entry[V])}
	})
	return st.(*state[K, V])
}
...
func (l *Loader[K, V]) runFetch(c *include.Ctx, st *state[K, V], mine []K, claimed []*entry[V]) error {
...
// New (nil fetch) always misses and caches nothing.
